...
export function ViewContract() {
    const [contractualization, setContractualization] = useState<Contractualization>()
    const [isLoading, setIsLoading] = useState(false)
    const { id } = useParams()

    const effectiveDate = contractualization && new Date(contractualization?.contract.effectiveDate).toLocaleDateString("pt-BR")
    const finishDate = contractualization && new Date(contractualization?.contract.finishDate).toLocaleDateString("pt-BR")
    const hadReajust = contractualization && contractualization.contract.prices.filter(e => e.isFromReajust).length >= 1
    const hadRenegotiation = contractualization && contractualization.contract.prices.filter(e => e.isFromRenegotiation).length >= 1
    ...
    useEffect(() => {
        if (id) {
            ...
            getContractualizationById(+id)
                ...
                .catch(() => {
                    showErrorToast({ message: "Não foi possível buscar contratualização" })
                })
                .finally(() => {
                    setIsLoading(false)
                })
                        ...
                        <>
                            ...
                            <div className="d-flex gap-2 flex-column">
                                        ...
                                        client && (
                                            <Col sm={12}>
                                                <div className="d-flex gap-2">
                                                    <img src={client.avatarUrl!} alt="Imagem de perfil do cliente" className="rounded" />
                                                    <div>
                                                        <p>
                                                            <b>Nome: </b>
                                                            {client.name!}
                                ...
                                <Row className="g-3 border rounded pb-3 px-3 rounded">
                                    ...
                                    {
                                        maintenanceTypes && maintenanceTypes.map((m) => {
                                            return (
