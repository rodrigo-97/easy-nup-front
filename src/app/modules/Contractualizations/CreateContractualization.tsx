...
export function CreateContractualization() {
    ...
    const [maintenanceTypes, setMaintenanceTypes] = useState<Array<MaintenanceTypeProps>>([
        {
            name: "",
            params: [
                {
    ...
    const schema = Yup.object().shape({
        ...
        finishDate: Yup.date().required("Campo obrigatório").min(new Date(), "A data precisa ser maior do que a atual (hoje)"),
        ...
        prices: Yup.object().shape({
            pf: Yup.number().required("Campo obrigatório").moreThan(0, "Campo obrigatório"),
            ust: Yup.number().required("Campo obrigatório").moreThan(0, "Campo obrigatório"),
            hh: Yup.number().required("Campo obrigatório").moreThan(0, "Campo obrigatório"),
        }).required("Campo obrigatório"),
        maintenanceTypes: Yup.array(Yup.object().shape({
            name: Yup.string().required("Campo obrigatório"),
            params: Yup.array(Yup.object().shape({
                name: Yup.string().required("Campo obrigatório"),
                fi: Yup.number().required("Campo obrigatório").moreThan(0, "Campo obrigatório")
            }))
        }))
    })
    ...
    function getParamMessageError(index: number, paramIndex: number, fieldName: string) {
        const hasMaintenanceTypes = errors.maintenanceTypes && errors.maintenanceTypes[index]

        if (hasMaintenanceTypes?.params) {
            if (fieldName === "name") return hasMaintenanceTypes.params[paramIndex]?.name?.message
            if (fieldName === "fi") return hasMaintenanceTypes.params[paramIndex]?.fi?.message
        }
    ...
    function removeMaintenanceType(index: number) {
        ...
        if (size > 1) {
            ...
            showErrorToast({ message: "Não é possível deixar uma contratualização sem um tipo de serviço" })
        }
    }
    ...
    function removeParamFromMaintenanceType(maintenanceTypeIndex: number, paramIndex: number) {
        ...
        if (size > 1) {
            ...
            showErrorToast({ message: "Não é possível deixar um tipo de serviço sem parâmetros" })
        }
    }
                        ...
                        <div className="app-bg p-4 mt-4">
                            <Row key={index}>
                                <div className="d-flex gap-3 col-12">
                                    <small className="mb-3 text-truncate">Tipo de serviço {index + 1}</small>
                                    <small
                                ...
                                <Col sm={12}>
                                    <FormGroup floating>
                                        <Controller
                                            ...
                                            render={({ field }) => {
                                                return (
                                                    <Input
                                                        {...field}
                                                        invalid={errors.maintenanceTypes && !!errors.maintenanceTypes[index]?.name}
                                                        placeholder="Nome"
                                                        type="text"
                                                    />
                                                )
                                        ...
                                        <FormFeedback>
                                            {errors.maintenanceTypes && errors.maintenanceTypes[index]?.name?.message}
                                        </FormFeedback>
                                    </FormGroup>
                                    <p >{errors.maintenanceTypes && !!errors.maintenanceTypes[index]?.message}</p>
                                </Col>
                                {
                                            ...
                                            <Row key={paramIndex}>
                                                <div className="d-flex justify-content-end">
                                                    <small
                                                        ...
                                                        title={`Clique para remover o parâmetro ${paramIndex + 1} do tipo de serviço ${index + 1}`}
                                                        onClick={() => removeParamFromMaintenanceType(index, paramIndex)}
                                                    >
                                                        remover
                                                    </small>
                                                </div>
                                                        ...
                                                        <Controller
                                                            ...
                                                            render={({ field }) => {
                                                                return (
                                                                    <Input
                                                                        {...field}
                                                                        className={!!getParamMessageError(index, paramIndex, 'name') ? 'is-invalid' : ''}
                                                ...
                                                <Col md={6}>
                                                    <FormGroup floating>
                                                        <NumberFormat
                                                            className={`form-control ${!!getParamMessageError(index, paramIndex, 'fi') ? 'is-invalid' : ''}`}
                                                            decimalSeparator=','
                                                            decimalScale={2}
                                                            allowNegative={false}
                                                            isAllowed={({ floatValue }) => {
                                                            ...
                                                            onValueChange={(values) => {
                                                                const { value } = values
                                                                setValue(`maintenanceTypes.${index}.params.${paramIndex}.fi`, +value)
                                                            }}
                                                        />
                                                        <Label>
                                                            Valor de ajuste %
