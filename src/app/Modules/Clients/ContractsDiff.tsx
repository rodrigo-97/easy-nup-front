...
export function ContractsDiff() {
  ...
  const lastVersion = contract && JSON.parse(contract.versions[0].serializedContract) as Contract
  ...
  function handleRejectUpdate() {
    contract && rejectUpdate({ clientId: contract?.client.id, companyId: contract?.company.id, contractId: contract?.id })
      .then(() => {
        handleGoBack()
        showSuccessToast({ message: "Solicitação rejeitada" })
      })
      .catch(() => showErrorToast({ message: "Não foi possível rejeitar alterações" }))
  }

  function handleAcceptUpdate() {
    contract && acceptUpdate({ clientId: contract?.client.id, companyId: contract?.company.id, contractId: contract?.id })
      .then(() => {
        handleGoBack()
        showSuccessToast({ message: "Solicitação aceita" })
      })
      .catch(() => showErrorToast({ message: "Não foi possível aceitar alterações" }))
  }
